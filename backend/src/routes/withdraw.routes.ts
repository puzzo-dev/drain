...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to create withdrawal', details: error.message });
    }
  },
);
      ...
      res.json({
        ...
        progress: {
          ...
          percentage: Math.round(
            (request.hopsCompleted / request.totalHops) * 100,
          ),
        },
        hops: request.mixingHops.map((hop: any) => ({
