...
export class MixingService {
  ...
  async createWithdrawalRequest(
    ...
    const totalHops = this.randomBetween(
      mixingConfig.minHops,
      mixingConfig.maxHops,
    );

    logger.info(
      `Creating withdrawal request with ${totalHops} hops for user ${userId}`,
    );
    ...
    if (!custodyWallet) {
      throw new Error(
        `No custodial wallet found for user ${userId} on ${network}`,
      );
    }
    ...
    logger.info(
      `Created ${totalHops} mixing hops for withdrawal request ${request.id}`,
    );
  ...
  async executeHop(hopId: string): Promise<void> {
    ...
    // Wait for delay
    logger.info(
      `Waiting ${hop.delaySeconds}s before executing hop ${hop.hopNumber}...`,
    );
    ...
    try {
      ...
      const asset = {
        ...
        name: hop.withdrawalRequest.symbol,
        amount: hop.amount,
